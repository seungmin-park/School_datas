...
완전 연결 계층으로 한 층의 모든 뉴련이 그 다음 층의 모든 뉴련과 연결된 상태
...
img = cv2.resize(img, (100,130)) # 이미지를 특정 사이즈로 재정의
img = cv.resize(img, (0,0), fx=0.5,fy=0.5) #이미지를 비율로 줄이기

print(img.shape)
# 이미지의 형태 출력 y,x,depth (1440, 1920, 3) -> 컬러 이미지일 경우 rgb 때문에 3출력
# 흑백 이미지일 경우 -> (1440, 1920)
```

#### Convolution(컨볼루션)

주변의 화소값들의 가중치를 곱해서 더한 후에 이 것을 새로운 화소값으로 하는 연산

```
컨볼루션 연산이 일어나면 기본적으로 이미지의 사이즈가 감소한다.
```

![image](https://user-images.githubusercontent.com/78605779/172377942-1d921284-d875-423d-88e7-b06f24d81e71.png)

### 다중 퍼셉트론(MLP) 파라미터 수 구하기

![image](https://user-images.githubusercontent.com/78605779/172376662-81b16878-f661-4e04-ae47-72f40a8e11ff.png)

### CNN (Convolution Neural Network) 컨볼루션 신경망

컨볼루션 신경망은 완전 연결 신경망과 달리 하위 레이어의 노드륻ㄹ과 상위 레이어의 노드들이 부분적으로만 연결되어 있다.

![image](https://user-images.githubusercontent.com/78605779/172400954-d5c95ce9-da31-41cc-b183-ba29e3f1554c.png)

컨볼루션 신경망은 모든 신경망 구조 중에서 가장 강력한 성능을 보여주는 신경망 중의 하나로서 **2차원 형태의 입력을 처리하기에 이미지 처리에 특히 적합**

`컨볼루션 신경망 출력 사이즈 구하기`

```
출력 사이즈 = (입력 사이즈 - 커널 사이즈) + 1

ex) 입력 사이즈: 7 x 7, 커널 사이즈: 3 x 3
-> (7 - 3) + 1 = 5
```

커널을 적용하는 거리, 즉 커널이 한번 움직일 때 이동하는 픽셀 수를 보폭(stride)이라 하는데 시험에서는 1로 고정

#### Padding

컨벌루션 연산시 가장 자리를 처리하기 위한 기법으로 default 값은 vaild로써 이미지의 사이즈가 줄어든다.

입력 사이즈를 그대로 유지하고 싶을 때 padding을 same으로 설정하면 된다. 생략시 vaild로 동작
...
![image](https://user-images.githubusercontent.com/78605779/172402940-cfb72317-5924-42ac-a253-7094c67e4ac7.png)
...
![image](https://user-images.githubusercontent.com/78605779/172403042-dc3a9604-6520-4b84-9da2-49992ca10ca4.png)

#### 풀링 (Pooling)

서브 샘플링이라고 하는 것으로 입력 데이터의 크기를 줄이는 것

```
시험에서는 (2,2) 즉 값을 절반으로 줄이는 것만 나올 예정.
소수점은 버림 계산
```
...
![image](https://user-images.githubusercontent.com/78605779/172405362-b5236d0d-c141-4144-80d0-4a9fd5b37546.png)

```markdown
# CNN 코드를 보고 Output Shape와 Param 계산 연습하기

풀링에는 Param이 존재하지 않는다.
```

### RNN (Recurrent Neural Network) 순환 신경망

순환 신경망는 보통 순차 데이터 (시계열 데이터)를 처리하는 신경망

```
시험에는 순환 신경망이 언제 쓰이는지, 혹은 예제가 RNN과 CNN중 어느 것이 적당한지
```
